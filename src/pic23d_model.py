...
class Model:
    ...
    def create_summarys(self):
        ...
        with tf.name_scope('scalars'):
            ...
            metrics = [self.tensor('dnn/loss/moments/Squeeze_1:0'),
                       ...
                       tf.reduce_mean(tf.squared_difference(self.lbls[:, 0, :10], self.tensor('dnn/prediction:0')[:, 0, :10]))]

            names = ['var', 'mean', 'max', 'min', 'mse', 'mse_top10_shape']
    ...
    def build_model(self):
        with tf.device(self.dev):
            self.input_placeholder, self.loss = self._build_model(self.imgs, self.optimizer)
            self.graph = tf.get_default_graph()
            self.saver = tf.train.Saver()
            self.create_summarys()
            self.merged = tf.summary.merge_all()

    def _build_model(self, data_input, optimizer_func):
            ...
            for i in range(self.conv_layers):
                ...
                layer_conv, weights_conv = \
                    new_conv_layer(input=out,
                                   ...
                                   filter_size=3,   #int(math.log(out_shape[0], 4))
                                   num_filters=filters,
        ...
        with tf.name_scope('train'):
            ...
            optimizer = optimizer_func(learning_rate=self.learn_rate, name='Adam')
            print(global_step)
    ...
    def predict(self, image):
        ...
        im_input = np.zeros((self.batchsize, self.size, self.size, 3))
        im_input[:im.shape[0], :im.shape[1], :im.shape[2], :im.shape[3]] = im

        pred = self.session.run(self.tensor('dnn/prediction:0'), feed_dict={self.tensor('dnn/input:0'): im_input})

        print(pred.shape)

        filename = os.path.basename(image)

        with open(f'{BASE_PATH}/{filename}.label', 'w') as f:
            for k in range(199):
                for j in range(2):
                    f.write(f'{pred[0,k,j]}')
                    if j == 0:
                        f.write('\t')
                    else:
                        f.write('\n')
